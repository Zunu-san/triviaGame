...
void game(int NumPlay, int NumRounds, int NumQues, string questions[125][5]);
void swap(string *a, string *b);
void swapInt(int *a, int *b);
void randomize(string array[125][5]);
void randomizeInt(int array);
void question(int QuesNum, string (*questions)[5], int *points, int i, int j, int k, int NumPlay);
void findwinner(int n, int *points, int *winnerpoints, int *winner);

int main()
{
    int NumPlay = 1, NumRounds = 1, NumQues = 1, newgame;
    bool gameflag = true;
    string questions[125][5] =
        {
            ...
            {"If you had pogonophobia what would you be afraid of?", "Desks", "Chairs", "Nails", "Beards"},
            {"Who would take silk as part of their job?", "Bartender", "Gardener", "Cashiers", "Barrister"},
            {"Who won an oscar for the African Queen?", "Cagney", "Barrymore", "Kelsey", "Bogart"},
            {"Who sang the theme song in 9 to 5?", "Carrie Underwood", "Reba McEntire", "Patsy Cline", "Dolly Parton"},
            {"What in business terms is the IMF?", "International Music Feed", "Individual Master File", "International Myeloma Foundation", "International Monetary Fund"},
            {"Ringo Star narrates which children's TV series?", "Inspector Mergou", "Lapitch the Little Shoemaker", "Hikayat Simsim", "Thomas the tank engine"},
            {"Which country grows the most fruit?", "Russia", "USA", "Bolivia", "China"},
            {"Which company is owned by Bill Gates?", "Apple", "Exxon", "Shell", "Microsoft"},
            {"What would you do with a maris piper?", "Call 911", "Hide under your bed", "Kill birds", "Eat it- it's a potato"},
            {"In Casablanca what is the name of the nightclub", "Jack's", "Moe's", "Gold's", "Rick's"},
            {"What was the first James Bond book?", "Goldfinger", "Moonraker", "From Russia With Love", "Casino Royal"},
            {"What kind of animal is a lurcher?", "Crow", "Raven", "Buffalo", "Dog"},
            {"What is the currency of Austria?", "Krone", "Krona", "Rouble", "Schilling"},
            {"What is the Islamic equal to the red cross?", "Al-Menbar Islamic Society", "Tehreek-e-Labbaik", "Ennahda Movement", "Red Crescent"},
            {"In fable who sold a cow for five beans?", "Totenkinder", "Geppetto", "Scarlet Robe", "Jack(and grew a beanstalk)"},
            {"How did Alfred Nobel make his money?", "He invented the Light Bulb", "Fortune cookie writer", "Full-time Netflix viewer", "He invented Dynamite"},
            {"Who was the first man to run a sub four minute mile?", "Yuri Milner", "John Landy", "Jim Ryun", "Roger Bannister"},
            {"What are Munroes?", "Cockroaches", "Polymers", "Rappers", "Mountains in Scotland"},
            {"Which car company makes the Celica?", "Ford", "Nissan", "Fiat", "Toyota"},
            {"Air Lingus is the national airline of which country?", "Austria", "Canada", "Democratic Republic of Congo", "Republic of Ireland or Eire"},
            {"Who discoverd Radium?", "Rutherford", "Pauling", "Pasteur", "The Curies"},
            {"What does an alopecia suffere lack?", "Teeth", "Nails", "Earlobes", "Hair"},
            {"Who painted The Haywain?", "Eugene Delacroix", "Claudio de Lorena", "Thomas Gainsborough", "John  Constable"},
            {"Triskadeccaphobia is the fear of what?", "Heights", "Insects", "Death", "Number 13"},
            {"What is a baby rabbit called?", "Calf", "Cub", "Pullet", "Kit or Kitten"}
        };
    ...
    while (gameflag)
    {
        cout << "How many are going to play? (1-5): " << endl;
        cin >> NumPlay;
        while (1 > NumPlay || NumPlay > 5)
        {
            if (cin.fail())
            {
                cout << "Please use a number, 1 to 5 players. Try again: " << endl;
                cin.clear();
            ...
            {
                cout << "Please select the adecuate range, only 1 to 5 players. Try again: " << endl;
                cin >> NumPlay;
            }
        }
        if (NumPlay > 1)
        {
            cout << "How many rounds do you want? (1-5): " << endl;
            cin >> NumRounds;
            while (1 > NumRounds || NumRounds > 5)
            {
                if (cin.fail())
                {
                    cout << "Please use a number, 1 to 5 rounds. Try again: " << endl;
                    cin.clear();
                    cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    cin >> NumRounds;
                }
                ...
                {
                    cout << "You can only choose between 1-5 rounds. Try again: " << endl;
                    cin >> NumRounds;
                }
            }
            ...
            while (1 > NumQues || NumQues > 5)
            {
                if (cin.fail())
                {
                    cout << "Please use a number, 1 to 5 questions. Try again: " << endl;
                    cin.clear();
                    cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    cin >> NumQues;
                }
        ...
        {
            cout << "And how many questions? (1-125): " << endl;
            cin >> NumQues;
            while (1 > NumQues || NumQues > 125)
            {
                if (cin.fail())
                {
                    cout << "Please use a number, 1 to 125 questions. Try again: " << endl;
                    cin.clear();
                    cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    cin >> NumQues;
                }
                ...
                {
                    cout << "Please, select from 1 to 125 questions. Try again: " << endl;
                    cin >> NumQues;
                }
            }
        }
...
void question(int QuesNum, string (*questions)[5], int *points, int i, int j, int k, int NumPlay)
{
    string correct[9] = {"Correct!!!", "Good Job!!!", "Well done!!!", "Niceeee", "Excelent!", "Very Good!", "Thats right!", "Awesome!", "Right Answer!"};
    string wrong[7] = {"Incorrect", "Thats not right", "Wrong answer", "*wrong answer buzz*", "Thats Right!!!, nah just kidding", "It's correct, except it doesn't"};
    int n[4] = {1, 2, 3, 4};
    int ans;
...
ENDTIMER:
    ...
    else
    {
        cout << "Answer: " << ans << ". " << *(*(questions + QuesNum) + n[ans - 1]) << endl
             << wrong[rand() % 7]  << endl
             << "The correct answer is '" << cans << "'" << endl;
        *(points + j) -= 5;
    }
}
...
void game(int NumPlay, int NumRounds, int NumQues, string questions[125][5])
{
    ...
    for (int i = 0; i < NumRounds; i++)
    {
        for (int j = 0; j < NumPlay; j++)
        {
            ...
            {
                question(i * NumQues * NumPlay + j * NumQues + k, questions, &points[0], i, j, k, NumPlay);
    ...
    else
    {
        if (NumPlay > 1)
        {
            ...
            for (int i = 0; i < NumPlay; i++)
            {
                if (points[i] == winnerpoints)
                {
                    cout << i + 1 << " with " << winnerpoints << " points." << endl;
                }
            }
        }
        else
        {
            cout << "You scored " << winnerpoints << " points." << endl;
        }
    }
}
